...
flags.DEFINE_string(
    "init_checkpoint", "/Users/trinhgiang/Downloads/multi_cased_L-12_H-768_A-12/tunning_model_500000.ckpt",
...
def get_sentence_lm_1(estimator, sentences):
    ...
    for (ppl, probs) in out:
        if (ppl < 25):
            predict_true.append(i)
        else:
            predict_false.append(i)
        i += 1

    return predict_true, predict_false


def get_sentence_lm_2(estimator, sentence):
    predict_examples = read_sentences(sentence)
    # predict_examples = read_examples(FLAGS.input_file)
    features, all_tokens = convert_examples_to_features(predict_examples,
                                                        FLAGS.max_seq_length, tokenizer)
...
def get_sentence_predict_2(sentence):
    tf.logging.set_verbosity(tf.logging.ERROR)
...
sentences = ['Tin học văn phòng Tiếng anh']
print(get_sentence_predict_2(sentences))
